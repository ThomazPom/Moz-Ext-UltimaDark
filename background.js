...
class uDarkC extends uDarkBackground {
...
class Listeners {
  ...
  static editBeforeRequestStyleSheet_sync(details) {
    ...
    filter.ondata = event => {
      details.dataCount++;
      Background.handleCSSChunk_sync(event.data, true, details, filter);
    };
    
    // onstop event handler
    filter.onstop = event => {
      if (details.rejectedValues.length > 0) {
        Background.handleCSSChunk_sync(null, false, details, filter);
      }
...
class Background {
  static handleCSSChunk_sync(data, verify, details, filter) {
  ...
  static getInjectCSS(resourcesPaths, actions = {}) {
  ...
  static registerCS(contentScriptRegister) {
    let defaultCS = {
  ...
  static setListener(initial) {
      let userSettings = uDark.userSettings;
      initial && Common.appCompat(userSettings);
      uDark.userSettings.properWhiteList = (userSettings.white_list || Background.defaultRegexes.white_list).split("\n").filter(Background.filterContentScript)
      uDark.userSettings.properBlackList = (userSettings.black_list || Background.defaultRegexes.black_list).split("\n").filter(Background.filterContentScript)
      uDark.userSettings.exclude_regex = (userSettings.black_list || Background.defaultRegexes.black_list).split("\n").map(x => x.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') // Sanitize regex
      ...
      if (!userSettings.disable_webext && userSettings.properWhiteList.length) {
        ...
        let contentScript = {
          js: [
            ...
            {
              code: `window.userSettings = ${JSON.stringify(uDark.userSettings)};`
            },
            {
              file: "background.js",
            },
        ...
        Background.registerCS({
          css: [{
            code: uDark.inject_css_override_top_only
          }],
          allFrames: false
        });
        Background.registerCS(contentScript);
        Background.registerCS({
          css: [{
            code: uDark.inject_css_override_top_only
          }],
  ...
  static install() {
    ...
    browser.runtime.onConnect.addListener(Background.portConnected);
    ...
    Promise.all([
      ...
      Background.getInjectCSS("/inject_css_suggested.css", {
        append: {
          inject_css_suggested: uDark,
        },
        edit_css: true
      }),
      Background.getInjectCSS("/inject_css_suggested_no_edit.css", {
        append: {
          inject_css_suggested: uDark
        }
      }),
      Background.getInjectCSS("/inject_css_override.css", {
        append: {
          inject_css_override: uDark
        },
        edit_css: true
      }),
      Background.getInjectCSS("/inject_css_override_top_only.css", {
        append: {
          inject_css_override_top_only: uDark
        },
        edit_css: true
      }),
      Background.getInjectCSS("/inject_css_override_no_edit.css", {
        append: {
          inject_css_override: uDark
        }
      }),
      new Promise(Background.getSettings)
    ]).then(x => console.info("CSS processed")).then(r => Background.setListener(true));
  ...
  static portConnected(connectedPort) {
...
if (browser.webRequest) {
  ...
  ContentScript.install();
  ContentScript.website_load();
}

console.info("UltimaDark", "Installed", uDark.edit_str("background:red"));