...
var uDark = {
    ...
    is_photo: (targetBitmap, editionStatus) => {
        ...
        editionStatus.pixelCount = pixelCount;
    ...
    calcStatsNoRun(editionStatus, ctx, canvas) {
        if (editionStatus.statsComplete) {
            editionStatus.theImageDataClamped8TMP.set(editionStatus.imageData.data);
            return;
        }
        const imageData = ctx.getImageData(0, 0, ctx.canvas.width, ctx.canvas.height);
        const src = imageData.data;                 // Uint8ClampedArray RGBA
        const buf = new ArrayBuffer(src.length);    // copie de sortie (on peut aussi travailler in-place si désiré)
        const out = new Uint8ClampedArray(buf);
        out.set(src);
        editionStatus.theImageDataClamped8TMP = out;
        ...
        editionStatus.moreOneColor = false;
        let _first = undefined;
        for (let i = 0; i < out.length; i += 4) {
            let r = out[i], g = out[i + 1], b = out[i + 2], a = out[i + 3];
            // Luma rapide 0..255
            const L = uDark.luma8_fast(r, g, b);
            const Lvis = L < a ? L : a;

            // Dans la boucle (r,g,b,a doivent être définis)
            if (editionStatus.moreOneColor !== true) {
                const number = ((a << 24) | (b << 16) | (g << 8) | r) >>> 0;
                if (_first === undefined) _first = number;
                else if (number !== _first) { editionStatus.moreOneColor = true; /* break; */ }
            }
            lightness_sum += Lvis;
            alpha_sum += a;
        ...
        editionStatus.isContrastBright = editionStatus.contrast_percent > .77 && editionStatus.avg_ligtness > 220 && editionStatus.max_lightness >= 204;
    ...
    background_image_edit_hook: async function (editionStatus, canvas, ctx, img, blob, details, imageURLObject, complement) {
        ...
        if (!editionStatus.isContrastBright && !editionStatus.editionConfidence >= 100 && editionStatus.moreOneColor === true) {
    ...
    edit_an_image: async function (details) {
        ...
        try {
            ...
            console.log("Image", "Invalid or disposed image", performance.now() - perf_start, details.url, details.requestId)
        ...
        let benchDirectAIPrediction = false;
        if (benchDirectAIPrediction) {
            let model = await loadLayerModelAndWeightsFromBuffers();
            let blob = (new Blob(imageBuffers));
            let imageBitmap = await createImageBitmap(blob, {
                resizeHeight: 64,
                resizeWidth: 64
            });
            const tensor = bitmapToTensor_tfmin(imageBitmap);
            ...
            var idx = pred.argMax(1).dataSync()[0];
        ...
        // blob.arrayBuffer().then((buffer) => {
        {
            ...
            if (benchDirectAIPrediction) {
                ...
                ].forEach(text => ctx.fillText(text, 7, 50 + 24 * i++));
